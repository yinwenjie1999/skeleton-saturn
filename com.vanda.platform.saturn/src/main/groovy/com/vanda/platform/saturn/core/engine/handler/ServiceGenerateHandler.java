...
/**
 * 该处理驱动负责根据完成的实体层扫描内容，按照设置的要求生成服务层代码
 * @author yinwenjie
 */
public abstract class ServiceGenerateHandler implements SaturnHandler {
  ...
  public void handle(SaturnContext context) {
    // TODO 在该方法中调用获得setEntityFilePath中设置的文件位置，并开始在指定位置生成服务层类
  }
  
  /**
   * 设置本生成驱动器不负责的domain域，也就是说这些指定域下服务层代码不由本驱动器负责<br>
   * 注意不能排除默认域（也就是没有设置域信息的实体模型定义）的生成。<br>
   * 如果不设置，就是默认参考所有域下的实体层定义，进行服务层的生成
   * @param domains
   */
